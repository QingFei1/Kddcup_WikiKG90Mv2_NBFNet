...
class WikiKG90M(data.KnowledgeGraphDataset):
    ...
    def split(self):
        train_set = self
        valid_set = WikiKG90MValid(self.valid_dict)
...
@R.register("datasets.WikiKG90MValid")
class WikiKG90MValid(torch_data.Dataset):  
    ...
    def __init__(self, test_dict):
        ...
        self.num_candidate = self.t_candidate.shape[-1]
        if "t" in test_dict:
            self.t_correct_index = test_dict["t"]
        else:
            self.t_correct_index = None
...
class WikiKG90MNumpy(torch_data.Dataset, core.Configurable):
    ...
    def __init__(self, path, feature_file=None, train_relation=None,
        ...
        self.test_dict = self.dataset.test_dict(mode = 'test-challenge')["h,r->t"]
        self.test_dict['t_candidate'] = self.test_candidate 

        self.valid_dict = self.dataset.valid_dict["h,r->t"] 
        self.valid_dict['t_candidate'] = self.valid_candidate
        self.valid_dict["t"]=self.valid_correct_index 
        ...
        self.valid_neighbor=valid_neighbor
        self.valid_expand=valid_expand
        if prune_isolated:
    ...
    def get_n_hop_graph_py(self, nodes, return_distance=False): 
        ...
        while queue:
            ...
            if expand >0 :
                ...
                if self.valid==True:
                    num_neighbor=self.valid_neighbor  
                for i in range(0, min(num_neighbor, degree)): 
                    if num_neighbor<degree:
                        choice=np.random.randint(0, degree)  #
                    else:
                        choice=i
                    index = offset + choice % degree
    ...
    def get_negative_t(self, h_index, t_index,r_index):
        neg_t_index = []
        neg_index=list(self.neg_list[r_index])
        neg_len=len(neg_index)
        t_index=set()

        if self.num_neg =="method1":
            if neg_len>500:
                while len(t_index)<self.num_negative:
                    select=np.random.randint(0,neg_len)
                    r_t_index=neg_index[select]
                    if r_t_index != h_index and r_t_index != t_index :
                        t_index.add(r_t_index)
                neg_t_index=list(t_index)
            ...
            if neg_len>=self.num_negative+2:
                ...
                neg_t_index=neg_index
                if h_index  in neg_t_index:
                    neg_t_index.remove(h_index)
                if t_index in neg_t_index:
                    neg_t_index.remove(t_index)
                if len(neg_t_index)==self.num_negative+1:
                    index = np.random.randint(0, self.num_negative+1)
                    del neg_t_index[index]

                for n in range(self.num_negative-len(neg_t_index)):
                    index = np.random.randint(0, self.num_entity)
                    neg_t_index.append(index)
...
@R.register("datasets.WikiKG90Mv2ValidNumpy") 
class WikiKG90MValidNumpy(torch_data.Dataset):

    def __init__(self, train_set, valid_dict):
        self.train_set = train_set
        self.hr = valid_dict["hr"]
        self.t_candidate = valid_dict["t_candidate"] 
        if "t" in valid_dict:
            self.t_correct_index = valid_dict["t"] 
        else:
            self.t_correct_index = None
    ...
    def __getitem__(self, index):
        ...
        t_candidate_index = self.t_candidate[index]  
        t_del1=np.array([-1])
        t_candidate_index=np.setdiff1d (t_candidate_index,t_del1,True)
        self.num_candidate = t_candidate_index.shape[-1]
        t_candidate_index_list=t_candidate_index.tolist()


        nodes = [h_index] + t_candidate_index_list 
        if self.train_set.use_entity_embedding:
            h_embedding = self.train_set.entity_embedding[h_index]
            t_embedding = self.train_set.entity_embedding[t_candidate_index]
        ...
        if self.train_set.node_feature is not None:
            ...
            t_feature = self.train_set.node_feature[t_candidate_index]
            # h_feature = np.tile([h_feature], (self.num_candidate, 1))
        
        self.train_set.valid=True
