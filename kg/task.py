...
class KnowledgeGraphEmbeddingKDDCup(tasks.KnowledgeGraphCompletion, core.Configurable):
    ...
    def forward(self, batch, all_loss=None, metric=None): 
        ...
        for criterion, weight in self.criterion.items():
            if criterion == "bce":
                ...
                loss = loss.mean(dim=-1)
            elif criterion == "red_gnn":
                max_n = torch.max(pred, 1, keepdim=True)[0]
                loss = torch.sum(- pos_pred + max_n + torch.log(torch.sum(torch.exp(pred - max_n),1))) 
            else:
    ...
    def evaluate(self, pred, target):
        ...
        for i in range(pred.shape[0]):
            score = pred[i]
            candidate = valid_candidate[i]
            sort_index = np.argsort(score)
            for j in range(1, 11):
                t_pred_top10[i][j - 1] = candidate[sort_index[-j]]
        ...
        for _metric in self.metric:
            if _metric == "mrr (lsc)":
                valid_dict = {}
                valid_dict['h,r->t'] = {'t': target, 't_pred_top10': t_pred_top10}
                evaluator = lsc.WikiKG90Mv2Evaluator()
                score = evaluator.eval(valid_dict)["mrr"]
